...
def train_model(model, train_dl, epochs, display_every=250):
    ...
    for e in range(epochs):
        ...
        if curr_loss < old_loss:
            ...
            torch.save(model.state_dict(), "vgg16-FULLTRAINED-best-model.pt")
            old_loss = curr_loss
...
def build_res_unet(n_input=1, n_output=2, size=SIZE):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    body = create_body(vgg16, pretrained=True, n_in=n_input, cut=-2)
...
def pretrain_generator(net_G, train_dl, opt, criterion, epochs):
    ...
    for e in range(epochs):
        ...
        if loss < last_loss:
            torch.save(net_G.state_dict(), "vgg16-pretraining-best-model.pt")
...
torch.save(model.state_dict(), "vgg16-FULLTRAINED-last-model.pt")
