...
train_idxs = rand_idxs[:9000] # choosing the first 8000 as training set
val_idxs = rand_idxs[9000:] # choosing last 2000 as validation set
...
def visualize(model, data, epoch, save=True):
    model.net_G.eval()
    with torch.no_grad():
    ...
    # plt.show()
    if save:
        fig.savefig(f"output/colorization_epoch_{epoch}.png")
    ...
    for e in range(epochs):
        ...
        for data in train_dl:
            ...
            if i % display_every == 0:
                ...
                visualize(model, data, e, save=True) # function displaying the model's outputs
...
def pretrain_generator(net_G, train_dl, opt, criterion, epochs):
    ...
    for e in range(epochs):
        ...
        if loss < last_loss:
            torch.save(net_G.state_dict(), "resnet18-pretraining-best-model.pt")
...
#torch.save(net_G.state_dict(), "vgg16-pretraining-last-model.pt")

#TRAINING:
net_G = build_res_unet(n_input=1, n_output=2, size=SIZE)
#net_G.load_state_dict(torch.load("vgg16-pretraining-best-model.pt", map_location=device))
model = MainModel(net_G=net_G)
train_model(model, train_dl, 200)

torch.save(model.state_dict(), "resnet18-FULLTRAINED-last-model.pt")
